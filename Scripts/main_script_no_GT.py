...
import SyntenyLink_wg_no_GT as wg
...
def main(gap_thresholds, min_block_lengths, y = 0):
    ...
    for key, value in dfs.items():
        ...
        wg.node_traverse_most_weighted_path(n_subgenomes,df_synteny, chains_file, blastn_file, C_df_new, num_blocks_main, first_letter_get)
        print("===========================================")
        ...
        for k in range(n_subgenomes):
            ...
            C_new.to_excel(f"C_new{k+1}.xlsx")
        ...
        # Change the column names
        df_subgenome.columns = [f"subgenome{i+1}" for i in range(n_subgenomes)]
        df_subgenome.to_excel(f"Final_result{key}.xlsx")
