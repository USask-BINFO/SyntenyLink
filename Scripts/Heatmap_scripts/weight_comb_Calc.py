...
import pickle
import re
import accuracy_heatmap_weight_comb as accur
import wandb
...
def node_traverse_most_weighted_path(n_subgenomes, subgenome_density_file_path, nodes, W1, W2, graph_input):
    '''
        ...
        for w2 in range(1, 21):
            ...
            for k in range(n_subgenomes):
                ...
                if k == 0:
                    ...
                    wandb.log({"acc_sub2": acc, "w1_sub2": w1, "w2_sub2": w2, "subgenome": k+1})
                elif k == 2:
                    wandb.log({"acc_sub3": acc, "w1_sub3": w1, "w2_sub3": w2, "subgenome": k+1})

                accuracies[w1-1][w2-1][k].append(acc)

    # Re-organize the accuracies list to have the correct dimensions
    new_accuracies = []
    for k in range(n_subgenomes):
        subgenome_acc = []
        for w1 in range(20):
            row_acc = []
            for w2 in range(20):
                row_acc.extend(accuracies[w1][w2][k])
            subgenome_acc.append(row_acc)
        new_accuracies.append(subgenome_acc)

    for k in range(n_subgenomes):
        # Generate heatmap for subgenome k
        accur.generate_heatmap(new_accuracies, k, 1.0, 1.0)
        
        # Get the highest accuracy value and its corresponding gap threshold and minimum block length values for subgenome k
        max_acc = max([max(row) for row in new_accuracies[k]])
        max_indices = [(i, j) for i, row in enumerate(new_accuracies[k]) for j, val in enumerate(row) if val == max_acc]
