...
class BP(object):
    ...
    def gap_calculation(self, ploidy_numpy, gap_threshold, minimum_block_length, n_sub):
        ...
        for key, np_array in ploidy_numpy.items():
            ...
            print(f"all_gaps: {all_gaps}, col_ind: {col_ind}, i: {i}")
            ...
            if n_sub == 2:
                ...
                i_vals = [
                    int(all_gaps[col_ind[k]][0, 0]) if all_gaps[col_ind[k]].size > 0 else float('inf')
                    for k in range(n_sub - 1)
                ]
                i = min(i_vals)

                if i == float('inf'):
                    raise ValueError("All gaps are empty for the selected columns. Check the input data or parameters.")
